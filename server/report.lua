...
RegisterNetEvent("astra_staff:takeReport")
AddEventHandler("astra_staff:takeReport", function(reportId)
    local source = source
...
RegisterNetEvent("astra_staff:closeReport")
AddEventHandler("astra_staff:closeReport", function(reportId)
    local source = source
    local xPlayer = ESX.GetPlayerFromId(source)
    if xPlayer.getGroup() == "user" then
        DropPlayer(source, "Vous n'avez pas la permission de faire cela")
